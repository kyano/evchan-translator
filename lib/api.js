 ...
 */
function withTimeoutSignal(signal) {
...
/**
 * Extract the chat completion content from an API response.
 * Shared helper to avoid duplicated response validation logic.
 * @param {Response} response - Fetch response
 * @param {string} label - Human-readable label for error messages
 * @returns {Promise<string>} The message content from the response
 */
async function extractChatContent(response, label) {
  ...
  if (!data.choices || !data.choices[0] || !data.choices[0].message) {
    throw new Error(`Invalid response format from ${label} API`);
  }
...
export function buildTranslationMessage(targetLang, text, sourceLang, pageTitle) {
  ...
  return `${context}Translate the following text ${langPhrase}. Return only the translated text. Do not add explanations, comments, or markdown formatting.
...
export function buildBatchTranslationMessage(texts, targetLang, sourceLang, pageTitle) {
  ...
  return `${context}Translate the following texts ${langPhrase}.
...
export async function translateText(
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      model,
      messages: [
        { role: 'user', content: buildTranslationMessage(targetLang, text, sourceLang, pageTitle) },
      ],
      max_tokens: 4096,
      temperature: 0.3,
      stream: false,
...
export async function translateTextBatch(
  ...
  const content = await extractChatContent(response, 'Batch translation');
