...
export async function translateTextBatch(
  ...
  let translations;
  try {
    translations = JSON.parse(stripMarkdownFences(data.choices[0].message.content));
  } catch {
...
export async function translateHtml(
  ...
  if (!response.ok) {
    throw new Error(`HTML translation failed: ${response.status} ${response.statusText}`);
  }

  const data = await response.json();

  if (!data.choices || !data.choices[0] || !data.choices[0].message) {
    throw new Error('Invalid response format from HTML translation API');
  }
